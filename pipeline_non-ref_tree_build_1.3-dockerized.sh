...
for i in ${id[@]}; do
    if [[ $i =~ "SRR" ]]; then
        if (find ./*$i*); then
            ...
            docker run -e i -e THREADS --rm=True -u $(id -u):$(id -g) -v $PWD:/data staphb/sratoolkit:2.9.2 /bin/bash -c \
            'fasterq-dump --skip-technical --split-files -t /data/tmp-dir -e ${THREADS} -O /data -p ${i}.sra'
